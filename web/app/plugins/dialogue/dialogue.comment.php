...
switch ($pid) {
  ...
  case 'paragraph-313':
    ...
    break;
  case 'paragraph-358':
    $anchor_id = 'conclusoes';
    break;
}

?><div class="comment-pp" <?php if ($anchor_id) : ?>id="<?php echo $anchor_id; ?>"<?php endif; ?>><p class="comment-text"><?php echo ($content);?><a class="comment-button" href="javascript:;"><i class="icon-comment"></i><?php if ($count > 0) { echo "$count "; } ?></a></p><div class="message"></div>
  <div class="comment-form"><div class="loading" style="display:none;"><strong>Enviando seu comentário</strong></div><?php global $current_user; get_currentuserinfo();
    if (comments_open ()) :
        if (is_user_logged_in()  ) : ?><form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform-<?php echo $pid; ?>"><input type="hidden" name="dialogue_comment_paragraph" value="<?php echo ($pid); ?>" /><input type="hidden" name="comment_post_ID" value="<?php echo ($post->ID); ?>" /><input type="hidden" name="comment" value="." /><h6><i class="icon-comment"></i>comentar este trecho</h6></form>
      <?php
        else:
      /*
          ...
          logged in
      */
      ?><input type="hidden" name="dialogue_comment_paragraph" value="<?php echo ($pid); ?>" /><input type="hidden" name="comment_post_ID" value="<?php echo ($post->ID); ?>" /><p>Para habilitar os comentários, por favor, faça seu <a href="<?php echo wp_login_url('/minuta/'); ?>" class="login-button">login</a>.</p></form>
<?php
        endif;
    else :
?>
